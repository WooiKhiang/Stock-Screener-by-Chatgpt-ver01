...
def _alpaca_tf(resolution: str) -> str:
    """
    Map UI resolution to Alpaca timeframe
    '5','15','30','60','D' -> '5Min','15Min','30Min','1Hour','1Day'
    """
    mapping = {
        "5": "5Min",
        "15": "15Min",
    ...
    return mapping.get(resolution, "1Hour")
...
def get_candles_alpaca(symbol: str, resolution: str = "60", lookback_days: int = 60) -> pd.DataFrame:
    ...
    Uses feed='iex' so it works on free paper accounts.
    """
    ...
    params = {
        ...
        "end": end.isoformat(),
        "limit": 10000,
        "feed": "iex",  # IMPORTANT: free paper accounts have access to IEX feed
        "adjustment": "all",
    }
...
def get_candles_finnhub(symbol: str, resolution: str = "D", lookback_days: int = 60) -> pd.DataFrame:
...
st.title("⚡ Auto Screener + Trader (Alpaca + Finnhub)")
st.caption("Scanner-only mode supported. Recommended timeframe: 1h for retail swing. Build: 2025-09-16")

with st.sidebar:
    st.header("Settings")
    # Data source toggle (Alpaca recommended; Finnhub free may block intraday)
    data_source = st.selectbox("Data source", ["Alpaca", "Finnhub"], index=0)
    ...
    resolution = st.selectbox("Bar timeframe", ["60", "30", "15", "5", "D"], index=0,
                              help="Use 'D' for Finnhub free daily; Alpaca works with 1Hour if you set feed=iex.")
    ...
    scanner_only = st.checkbox("Scanner-only mode (no broker calls)", value=True,
                               help="Skip broker account/positions/clock calls and disable order placement.")
    ...
    with st.expander("🔧 Debug"):
        st.write("Finnhub key loaded?", bool(FINNHUB_KEY))
        st.write("Alpaca key loaded?", bool(ALPACA_KEY))
        if st.button("Test Finnhub (AAPL, D)"):
            try:
                df_test = get_candles_finnhub("AAPL", resolution="D", lookback_days=30)
                st.write("Rows:", len(df_test))
                st.dataframe(df_test.tail(5))
            except Exception as e:
                st.error(str(e))
        if st.button("Test Alpaca data (AAPL, 1h)"):
            try:
                df_test = get_candles_alpaca("AAPL", resolution="60", lookback_days=10)
                st.write("Rows:", len(df_test))
                st.dataframe(df_test.tail(5))
            except Exception as e:
                st.error(str(e))

# Broker panels (optional; only when not in scanner-only and have keys)
...
if run_scan:
    ...
    for i, sym in enumerate(tickers, start=1):
        try:
            if data_source == "Alpaca":
                ...
                df = get_candles_finnhub(sym, resolution=resolution, lookback_days=lookback_days)
    ...
    # ---- Trade plan (optional) ----
    # Buy passed names until max_positions is reached; skip if already have position
    if buys:
        st.markdown("### Trade Plan")
        held = (
            {p.get("symbol"): float(p.get("qty", 0)) for p in alpaca_list_positions()}
            if (ALPACA_KEY and ALPACA_SECRET and not scanner_only)
            else {}
        )
...
st.markdown("---")
st.caption(f"Build finished at {utcnow_iso()} — Keep PAPER trading while testing.")
