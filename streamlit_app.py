# streamlit_app.py — Dynamic Universe (Yahoo) → Google Sheet (fast, simple, accurate)
...
# Optional overrides via secrets/env (won’t slow down)
...
# -----------------------
# Symbol directory (NASDAQ + NYSE), filter out funds/derivs
# -----------------------
...
def fetch_daily(tickers: List[str], period: str = "6w", chunk: int = 250, threads_on: bool = True) -> Dict[str, pd.DataFrame]:
    """Download recent daily bars. 6w is enough for EMA20 + last 5 bars."""
    ...
    for i in range(0, total, CHUNK):
        ...
        try:
            raw = yf.download(" ".join(sub), interval="1d", period=period,
                              group_by="ticker", auto_adjust=False, progress=False, threads=threads_on)
        except Exception:
...
# -----------------------
# Universe builder ($-flow share, price band, momentum, volume exclusion)
# -----------------------
...
def build_universe_flow_rank(
    ...
    daily = fetch_daily(pool, period="6w", chunk=yf_chunk, threads_on=yf_threads)
    t2 = time.perf_counter()

    candidates: List[Tuple[str,float,float,float]] = []  # (sym, adv, last_vol, last_close)
    for sym, df in daily.items():
        if df is None or df.empty or len(df) < max(daily_bars + 1, 21):
            continue
    ...
    total_adv = float(sum(adv for _, adv, *_ in candidates)) or 1.0
    scored = [(sym, adv/total_adv, adv) for (sym, adv, *_ ) in candidates]
    ...
    if len(scored) < max(100, int(0.5*n_top)) and gentle_momentum:
        ...
        if candidates2:
            ...
            total_adv2 = float(sum(adv for _, adv, *_ in candidates2)) or 1.0
            scored = [(sym, adv/total_adv2, adv) for (sym, adv, *_ ) in candidates2]
            scored.sort(key=lambda x: (x[1], x[2]), reverse=True)

    top_syms = [sym for sym, _share, _adv in scored[:n_top]]
    return top_syms, {"evaluated": len(daily), "kept": kept, "after_excl": after_excl, "t_sym": t1-t0, "t_dl": t2-t1}
...
st.caption("Hardcoded Sheet ID/Name. Credentials in secrets. Data via Yahoo Finance daily bars. Fast mode = fewer symbols + threaded download + bigger batches.")
